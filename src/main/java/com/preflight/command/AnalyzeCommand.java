...
public class AnalyzeCommand {
    ...
    public String scan(
        ...
        try {
            ...
            if (repoRoot == null) {
                return red("Error: Not a git repository: " + path);
            }
            ...
            if (!response.hasIssues()) {
                return green("V No critical issues found. Code looks clean!");
            }
    ...
    public String fix(
        ...
        try {
            ...
            if (!response.hasIssues()) {
                System.out.println(green("V No critical issues found. Proceeding with commit."));
                return "";
            }
            ...
            long applied = results.stream().filter(result -> result).count();
            long failed = results.stream().filter(result -> !result).count();

            StringBuilder sb = new StringBuilder();
            sb.append(green(String.format("V Applied %d fix(es) successfully.", applied)));
            if (failed > 0) {
    ...
    public String installHook(
        ...
        try {
            ...
            return green("V Pre-commit hook installed at " + repoRoot + "/.git/hooks/pre-commit\n"
    ...
    private String resolveAndValidateRepo(String path) {
        if (!gitTool.isGitRepository(path)) {
            return null;
        }
    ...
    private String red(String t) {
        return ANSI_RED + t + ANSI_RESET;
    }

    private String green(String t) {
        return ANSI_GREEN + t + ANSI_RESET;
    }

    private String yellow(String t) {
        return ANSI_YELLOW + t + ANSI_RESET;
    }

    private String cyan(String t) {
