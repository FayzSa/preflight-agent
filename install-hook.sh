...
# Installed by ai-fix (https://github.com/fayzsa/preflight-agent)
...
if command -v ai-fix &>/dev/null; then
...
elif [ -f "./target/preflight-agent-1.0.0.jar" ]; then
  java -jar ./target/preflight-agent-1.0.0.jar fix --auto
  EXIT_CODE=$?
  if [ $EXIT_CODE -ne 0 ]; then
    echo "[ ai-fix ] Critical issues detected. Commit blocked."
    exit 1
  ...
  echo "[ ai-fix ] Install: https://github.com/fayzsa/preflight-agent"
fi
